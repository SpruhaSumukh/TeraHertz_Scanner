function thresh_im = combinations3_for_nn( image,  number_of_nn)
...
for l = number_of_nn+1:x-number_of_nn 
    for m = number_of_nn+1:y-number_of_nn
            ...
            new_img(l,m) = median(new_img_intr(1:a));
            median_image(l,m) = new_img(l,m);
            if(median_image(l,m)>image(l,m))
                thresh_im(l,m) = 0;
